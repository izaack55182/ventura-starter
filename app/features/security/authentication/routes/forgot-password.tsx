...
import { EmailSchema, UserNameSchema } from '@/features/user/schemas'
import type { Route } from '@/rr/features/security/authentication/routes/+types/forgot-password'
import { db } from '@/utils/db.server'
import { checkHoneypot } from '@/utils/honeypot.server'
import { prepareVerification } from '../server/verify.server'
	...
	const submission = await parseWithZod(formData, {
		schema: ForgotPasswordSchema.superRefine(async (data, ctx) => {
			const user = await db.query.user.findFirst({
				where: (user, { eq, or }) =>
					or(eq(user.email, data.usernameOrEmail), eq(user.username, data.usernameOrEmail)),
				columns: { id: true },
			})
			if (!user) {
				ctx.addIssue({
