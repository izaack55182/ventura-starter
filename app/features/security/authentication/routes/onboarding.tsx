...
import {
	...
	sessionKey,
	signup,
} from '@/features/security/authentication/server/auth.server.ts'
import { authSessionStorage } from '@/features/security/authentication/server/session.server.ts'
...
export async function action({ request }: Route.ActionArgs) {
	...
	return redirectWithToast(
		...
		{ type: 'success', title: 'Welcome', description: 'Thanks for signing up!' },
		{ headers }
	)
}
...
export default function OnboardingRoute({ loaderData, actionData }: Route.ComponentProps) {
	...
	const [form, fields] = useForm({
		...
		lastResult: (actionData as any)?.result,
		onValidate({ formData }) {
	...
	return (
		<Page variant="auth" align="center" className="mt-15 md:mt-25">
			<PageHeader className="text-center">
				<PageTitle>Welcome aboard!</PageTitle>
				<PageDescription className="break-all">{(loaderData as any)?.email}</PageDescription>
			</PageHeader>
